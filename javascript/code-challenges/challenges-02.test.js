...
const raisedToTheThird = (arr) => {
  ...
  let newArr = [];
  arr.forEach(element => {
    let poweredNum = (Math.pow(element, 3));
    newArr.push(poweredNum);
  });
  return newArr;
};
...
the array and returns a new array. The returned array should contain the result of raising 2 to
the power of the original input element.
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let poweredNum = (Math.pow(2, arr[i]));
    newArr.push(poweredNum);
  }
  return newArr;
};
...
const forEachTwoToThe = (arr) => {
  ...
  let newArr = [];
  arr.forEach(element => {
    let poweredNum = (Math.pow(2, element));
    newArr.push(poweredNum);
  });
  return newArr;
};
